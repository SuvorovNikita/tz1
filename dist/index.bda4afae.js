...
function buttonTopCheck() {
    ...
    if (document.documentElement.scrollHeight !== document.documentElement.offsetHeight) el.classList.add("button-top__hidden");
    else el.classList.remove("button-top__hidden");
}
buttonTopCheck();
...
function updateData() {
    fetch("data-1.json").then((response)=>response.json()).then((data)=>{
...
function renderData(data, isUpdate) {
    ...
    if (isUpdate) {
        while(tplTableEl.parentElement.children.length > 1)tplTableEl.parentElement.removeChild(tplTableEl.parentElement.lastChild);
        while(tplCardEl.parentElement.children.length > 1)tplCardEl.parentElement.removeChild(tplCardEl.parentElement.lastChild);
    }
